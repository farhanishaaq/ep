...
//               ->join('comments','comments.doctor_id','=','doctors.id')
            ->select('medical_specialties.name AS specialityName','doctors.id','min_fee', 'max_fee', 'code', 'title', 'qualifications.description AS qualificationsDescription', 'institute', 'fname', 'lname', 'full_name', 'dob', 'gender', 'additional_info', 'phone', 'address', 'email', 'photo','experience','doctors.affiliation AS doctorAffiliation','user_type','username','password','city_id','cnic','doctors.status AS doctorStatus')
            ->where('users.id', '=', $id)
            ->groupBy('users.id')
            ->get();
